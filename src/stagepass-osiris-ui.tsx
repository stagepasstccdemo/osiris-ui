...
export const Navigate: React.FC<IPropsNavigate> = (props: IPropsNavigate) => {
  ...
  return (
    <button type="button" name={props.path} onClick={redirect} style={{cursor: "pointer", background: 'green', color: "white", padding: "20px 40px", border: "none"}}>{props.children}</button>
  )
}